...
MRK_STATE_ORD = {x: i for i, x in enumerate(["C", "N", "O", "ES", "ESH"])}
...
class LightingExportTemplate(models.Model):
    ...
    def generate_dict(self, obj, header, hide_empty_fields=True):  # noqa: C901
        ...
        for field, meta in header.items():
            ...
            conv_func = (
                lambda x: meta["conv_code"] and safe_eval(meta["conv_code"] % x) or x
            )
            key_fields = []
            field_d = {}
            has_value = False
            ...
            for lang in meta_langs:
                ...
                if meta["type"] == "selection":
                    ...
                    datum1 = []
                    for x in datum.sorted(
                        lambda x: not order_field
                        ...
                        if subfield_l:
                            ...
                            if key_field_name:
                                key_fields_l = x.with_context(lang=None).mapped(
                                    key_field_name
                                )
                                if key_fields_l:
                    ...
                    if multivalue_method == "by_field":
                        if not meta["translate"]:
                            ...
                            for prefix, field_d in field_d.items():
                                ndxs = key_fields or [
                                    "%02d" % x for x in range(1, len(field_d) + 1)
                                ]
                        ...
                        if not meta["translate"]:
                            ...
                            if self.lang_field_format == "postfix":
                                for lang, datum in field_d.items():
                                    obj_d["%s_%s" % (field, lang)] = separator.join(
                                        map(str, datum)
                                    )
                            elif self.lang_field_format == "json":
                                obj_d[field] = field_d
    ...
    def _generate_products(self, header, object_ids, hide_empty_fields):
        ...
        for i, object_id in enumerate(object_ids, 1):
            ...
            if (i % th) == 0:
                _logger.info(
                    "%s Template: - Progress products generation %i%%"
                    % (self.name, int(i / n * 100))
                )
    ...
    def _generate_bundles(self, template_d):
            ...
            if is_bundle_template:
                ...
                if products_required:
                    ...
                    bundle_d[template_name].update(
                        {
                            "templates": sorted(
                                list(set(products_required.mapped("finish_group_name")))
                            )
                        }
                    )
                    ...
                    attachment_ids = (
                        products.mapped("attachment_ids")
                        .filtered(
                            lambda x: x.type_id.is_image
                            and x.type_id in attachment_order_d.keys()
                        )
                        .sorted(
                            lambda x: (attachment_order_d[x.type_id], x.sequence, x.id)
                        )
                    )
                    ...
                    if not attachment_ids:
                        attachment_ids = (
                            products_required.mapped("attachment_ids")
                            .filtered(
                                lambda x: x.type_id.is_image
                                and x.type_id in attachment_order_d.keys()
                            )
        ...
        _logger.info(
            "%s Template: Bundle products successfully generated..." % self.name
        )
        return bundle_d

    def _generate_templates(self, template_d, objects_d):
        _logger.info(
            "%s Template: Generating configurable products (grouped by finish)..."
            % self.name
        )
            ...
            if len(v) > 1:
                ...
                for lang in self.lang_ids:
                    lang_description = (
                        ...
                        ._generate_description(show_variant_data=False)
                    )
                    if lang_description:
                ...
                for f in fields:
                    for product in products.sorted(
                        lambda x: (
                            MRK_STATE_ORD.get(x.state_marketing, len(MRK_STATE_ORD)),
                            x.sequence,
                        )
                    ):
        ...
        _logger.info(
            "%s Template: Configurable products successfully generated..." % self.name
        )
    ...
    def _generate_groups(self, photo_group_d, objects_d):
        ...
        for group, products in photo_group_d.items():
            ...
            if not any(products.mapped("is_composite")) and not any(
                products.mapped("is_required_accessory")
            ):
                group_d = {}
                ...
                group_d.update(
                    {
                        "product": sorted(
                            list(set(products.mapped("finish_group_name")))
                        ),
                    }
                )
        ...
        _logger.info(
            "%s Template: Product groups successfully generated..." % self.name
        )
        return groups_d
        ...
        if families:
            ...
            for family in families.sorted(lambda x: x.sequence):
                ...
                if family.attachment_ids:
                    attachments = family.attachment_ids.filtered(
            ...
            _logger.info(
                "%s Template: Family data successfully generated..." % self.name
            )
            return family_ld
    ...
    def _generate_categories(self, categories):  # noqa: C901
        ...
        for category in categories.sorted(lambda x: x.sequence):
            ...
            if category.attachment_ids:
                ...
                # brands OLD
                brand_attach_d = {}
                brand_attachments = category.attachment_ids.filtered(
                    lambda x: x.brand_id
                ).sorted(lambda x: (x.sequence, x.id))
                ...
                # location OLD
                location_attach_d = {}
                location_attachments = category.attachment_ids.filtered(
                ...
                if not category_attach_d:
                    global_attach_d = {}
                    global_attachments = category.attachment_ids.sorted(
                ...
                # brands NEW
                brand_attach_d = {}
                brand_attachments = category.attachment_ids.sorted(
                    lambda x: (x.sequence, x.id)
                )
                for a in brand_attachments:
                ...
                # location NEW
                location_attachments = category.attachment_ids.sorted(
                    lambda x: (x.sequence, x.id)
                )
                location_attach_d = {}
                for la in location_attachments:
                    location_code = (
                        not la.location_default and la.location_id.code or None
                    )
        ...
        for line in self.field_ids.sorted(lambda x: x.sequence):
            ...
            for lang in self.lang_ids:
                item_lang = (
                    ...
                    .with_context(lang=lang.code)
                    .fields_get([field_name], ["type", "string", "selection"])
                )
                if item_lang:
            ...
            if item:
                ...
                item["multivalue_separator"] = (
                    line.multivalue_method == "by_separator"
                    and line.multivalue_separator
                    or None
                )
                item["multivalue_order"] = (
                    line.multivalue_method and line.multivalue_order or None
                )
    ...
    def _groups_by_finish(self, object_ids):
        ...
        _logger.info(
            "%s Template: Dictionary of groups by finish successfully generated."
            % self.name
        )
        return template_d
    ...
    def _groups_by_photo(self, object_ids):
        ...
        _logger.info(
            "%s Template: Dictionary of groups by photo successfully generated."
            % self.name
        )
