...
class LightingProduct(models.Model, mixin.LightingReviewMixin):
    ...
    def _get_toreview_count_fields(self):
        return ['toreview_count'] + ['%s.toreview_count' % x for x in self._toreview_related_models]

    @api.depends(_get_toreview_count_fields)
    def _compute_toreview_fields_count(self):
        for rec in self:
            rec.toreview_fields_count = \
                sum([sum(rec.mapped(f)) for f in rec._get_toreview_count_fields()])

    toreview_fields_count = fields.Integer(string="To review",
                                           compute="_compute_toreview_fields_count",
                                           store=True)

    # to review fields
    toreview_lifetime = fields.Boolean(string=mixin.get_string('Lifetime (h)'),
                                       help=mixin.TOREVIEW_HELP,
                                       track_visibility='onchange')

    toreview_led_lifetime_l = fields.Boolean(string=mixin.get_string('LED lifetime L'),
                                             help=mixin.TOREVIEW_HELP,
                                             track_visibility='onchange')
