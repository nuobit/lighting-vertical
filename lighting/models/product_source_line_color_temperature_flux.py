...
class LightingProductSourceLineColorTemperatureFlux(models.Model):
    ...
    color_temperature_value = fields.Integer(
        compute="_compute_color_temperature_value", store=True
    )
    ...
    nominal_flux = fields.Float(string="Nominal flux", required=True)
    total_flux = fields.Float(string="Total flux", help="Luminaire total nominal flux")
    ...
    source_line_id = fields.Many2one(
        comodel_name="lighting.product.source.line", required=True, ondelete="cascade"
    )
