...
class LightingProductCategoryAttachment(models.Model):
    ...
    brand_id = fields.Many2one(
        comodel_name="lighting.catalog", ondelete="cascade", string="Brand"
    )
    brand_default = fields.Boolean(string="Brand default")

    location_id = fields.Many2one(
        comodel_name="lighting.product.location", ondelete="cascade", string="Location"
    )
    location_default = fields.Boolean(string="Location default")

    category_id = fields.Many2one(
        comodel_name="lighting.product.category", ondelete="cascade", string="Category"
    )
