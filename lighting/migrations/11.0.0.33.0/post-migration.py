...
def migrate(env, version):
    ...
    for i, (id, install_location) in enumerate(rows, 1):
        if install_location:
            ...
            location_ids = []
            for e_id in elems:
                model, location_id = env["ir.model.data"].get_object_reference(
                    "lighting", e_id
                )
                location_ids.append(location_id)

            product = Product.browse(id)
            product.write({"location_ids": [(6, False, location_ids)]})

        if (i % 50) == 0:
            _logger.info(
                "    Progress converting old Location values %i%%" % int(i / n * 100)
            )
