...
def migrate(env, version):
    ...
    source_lines = (
        env["lighting.product.source.line"]
        .search(
            [
                "|",
                "|",
    ...
    for i, sl in enumerate(source_lines, 1):
        _logger.info(
            "Migrating source line of product %s %i/%i"
        ...
        if len(fluxes) == 2 and len(ctemps) == 1:
            ...
            raise Exception(
                "Line %i: Not matching K-lm pairs '%s' vs '%s'"
        ...
        for ct, flux in zip(ctemps, fluxes):
            ...
            if not ctf:
                ...
                if ctf_err:
                    raise Exception(
                        "Found partial match on source line %i, reference %s "
                        ...
                        % (
                            ...
                            ct.value,
                            flux.value,
                            ctf_err.color_temperature_id.value,
                            ctf_err.flux_id.value,
                        )
                    )
                ct_values.append(
                    {
