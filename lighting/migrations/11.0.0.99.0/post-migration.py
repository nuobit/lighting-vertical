...
def migrate(env, version):
    ...
    env.cr.execute("""  alter table product_template add column x738374_lighting_product_id int4""")
    env.cr.execute("""  insert into product_template ("name", default_code, type, categ_id, uom_id, uom_po_id, 
                        ...
                        from lighting_product p""", (category.id, uom.id, uom.id, now, now))
    env.cr.execute("""  insert into product_product (default_code, product_tmpl_id, active, create_date, write_date)
                        select t.default_code, t.id, t.active, t.create_date, t.write_date
                        from product_template t""")
    env.cr.execute("""  update lighting_product l
                        set odoo_id = p.id
                        from product_template t, product_product p
                        where t.id = p.product_tmpl_id and 
                              t.x738374_lighting_product_id = l.id""")
    env.cr.execute("""  alter table product_template drop column x738374_lighting_product_id""")
