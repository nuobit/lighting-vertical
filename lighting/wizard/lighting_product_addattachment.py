    ...
    def add_attachment(self):
        ...
        for product in products:
            ...
            for attach in product.attachment_ids.filtered(
                    lambda x: x.datas_location == self.datas_location and x.type_id == self.type_id):
            ...
            if attach_grouped:
                for gtype, attachs in attach_grouped.items():
                    if gtype == 'bydatas':
                        errors.setdefault(product.id, {'product': product})['same_checksum_match'] = attachs
                    elif gtype == 'byfname':
                        if len(attachs) > 1:
                            errors.setdefault(product.id, {'product': product})['more_than_one_fname_match'] = attachs
                        elif len(attachs) == 1:
                            if self.overwrite:
        ...
        for data in errors.values():
            ...
            if 'same_checksum_match' in data:
                msg0.append(_('Already exists other attachment with exactly the same content: %s') %
                            ', '.join({x.display_name for x in data['same_checksum_match']}))
            if 'more_than_one_fname_match' in data:
                msg0.append(_('More than one attachment found with the same name: %s') %
                            ', '.join({x.display_name for x in data['more_than_one_fname_match']}))
