...
def post_init_hook(cr, registry):
    ...
    tables = [
        ...
        ("sapb1_backend_catalog_map", "sapb1_lighting_backend_catalog_map",
         "sapb1_lighting_backend_catalog_map_id_seq"),
        ("sapb1_backend_lang_map", "sapb1_lighting_backend_lang_map", "sapb1_lighting_backend_lang_map_id_seq"),
        ("sapb1_backend_state_marketing_map", "sapb1_lighting_backend_state_marketing_map",
         "sapb1_lighting_backend_state_marketing_map_id_seq"),
        ("sapb1_light_product", "sapb1_lighting_product", "sapb1_lighting_product_id_seq"),
    ]

    models = [x[0].replace('_', '.') for x in tables]
    states = ['installed', 'to upgrade', 'to remove']
    if env['ir.module.module'].search([('name', '=', 'connector_sapb1')]).state in states and env[
        'ir.model'].search_count([('model', 'in', models)]) == len(models):
