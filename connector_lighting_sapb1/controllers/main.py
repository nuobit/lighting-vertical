...
class SAPB1LightingController(http.Controller):
    ...
    def download_doc_refund(self, doctype=None, cardcodenum=None, docnum=None):
        if doctype == "order":
            return werkzeug.exceptions.UnprocessableEntity(
                "An order cannot be refundable"
            )
        return self.download_doc(
    ...
    def download_doc(self, doctype=None, cardcodenum=None, docnum=None, refund=False):
        ...
        # get document data
        cursor.execute(sql, {"cardcode": cardcode, "docnum": docnum})
        ...
        for row in cursor:
            ...
            t_time = time.strptime("{:06d}".format(time_int), "%H%M%S")
            dt = dt.replace(
                hour=t_time.tm_hour, minute=t_time.tm_min, second=t_time.tm_sec
            )
        ...
        if not filematch:
            return werkzeug.exceptions.NotFound(
                "Document %i not found. Administrator notified" % docnum
            )
