...
class LigthingSAPB1ProductImportMapper(Component):
    ...
    def ean_codebar(self, record):
        return {"ean": record["CodeBars"] and record["CodeBars"].strip() or None}
    ...
    def stock(self, record):
        ...
        values["available_qty"] = (
            available_qty > 0 and available_qty or 0
        ) + capacity_qty
    ...
    def cost(self, record):
        return {
            ...
            "cost_currency_id": self._get_currency_id_by_name(
    ...
    def description_manual(self, record):
        return {
            "description_manual": record["ItemName"]
            and record["ItemName"].strip()
            or None
        }
    ...
    def state_marketing(self, record):
        u_acc_obsmark = (
            record["U_ACC_Obsmark"] and record["U_ACC_Obsmark"].strip() or None
        )
        if u_acc_obsmark:
            state_marketing = self.backend_record.state_marketing_map.filtered(
                lambda x: x.sap_state_reference == u_acc_obsmark
            ).state_marketing
    ...
    def family_ids(self, record):
        ...
        if not familia:
            ...
            if not families:
                ...
                if not references:
                    references = self._get_sibling_reference(
                        item_code, "^([A-Z0-9][0-9]{2})[A-Z]-.+$"
                    )
                if references:
                    reference_groups = {}
                    for r in references:
                    ...
                    if reference_groups:
                        families = sorted(
                            reference_groups.items(), key=lambda x: x[1], reverse=True
                        )[0][0]
        else:
    ...
    def category_id(self, record):
        category = None
        aplicacion = (
            record["U_U_aplicacion"] and record["U_U_aplicacion"].strip() or None
        )
        if not aplicacion:
            references = self._get_sibling_reference(
                record["ItemCode"].strip(), "^(.+)-[^-]{2}$"
            )
            if references:
                ...
                if len(category) > 1:
                    raise Exception(
                        _(
                            "The other variants have more than one category '%s', "
                            "it's not possible to infer the category."
                        )
                        % (category,)
                    )
        else:
            ...
            category = (
                self.env["lighting.product.category"]
                .with_context(lang="es_ES")
                .search([("complete_name", "=ilike", complete_name)])
            )
            if len(category) > 1:
                raise Exception(
                    _("Multiple Category %s found")
                    % (", ".join(category.mapped("name")),)
                )
        ...
        if not category:
            ...
            if catalog_name == ACCESSORY_CATALOG:
                ...
                if len(category) > 1:
                    raise Exception(
                        _("Multiple Accessory category %s found")
                        % (", ".join(category.mapped("name")),)
                    )
    ...
    def finalize(self, map_record, values):
        if (
            values.get("family_ids")
            and values.get("category_id")
