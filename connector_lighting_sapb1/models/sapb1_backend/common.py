...
class SapB1LightingBackend(models.Model):
    _name = "sapb1.lighting.backend"
    _inherit = "connector.backend"

    _description = "SAP B1 Lighting Backend Configuration"
    _order = "sequence"
    ...
    state_marketing_map = fields.One2many(
        ...
        comodel_name="sapb1.lighting.backend.state.marketing.map",
        inverse_name="backend_id",
    )

    catalog_map = fields.One2many(
        string="Catalog Map",
        comodel_name="sapb1.lighting.backend.catalog.map",
        inverse_name="backend_id",
    )

    language_map = fields.One2many(
        string="Langauage Map",
        comodel_name="sapb1.lighting.backend.lang.map",
        inverse_name="backend_id",
    )
    ...
    def import_products_since(self):
        for rec in self:
            self.env["sapb1.lighting.product"].with_delay().import_products_since(
                backend_record=rec
            )
    ...
    def export_products_since(self):
        for rec in self:
            self.env["sapb1.lighting.product"].with_delay().export_products_since(
                backend_record=rec
            )
