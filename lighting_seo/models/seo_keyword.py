...
class LightingSEOKeyword(models.Model):
    ...
    application_count = fields.Integer(
        compute="_compute_counts", string="Application(s)"
    )
        ...
        for record in self:
            for field, model in maps:
                count = self.env[model].search_count(
                    [("seo_keyword_ids", "=", record.id)]
                )
                setattr(record, field, count)

    all_product_count = fields.Integer(
        compute="_compute_all_product_count", string="All product(s)"
    )
    ...
    def _compute_all_product_count(self):
        ...
        for record in self:
            ...
            for field, model in maps:
                ids = (
                    record.env[model]
                    .search([("seo_keyword_ids", "=", record.id)])
                    .mapped("id")
                )
                product_ids.update(
                    record.env["lighting.product"].search([(field, "in", ids)])
                )
    ...
    def unlink(self):
        ...
        for m in models:
            ...
            if records:
                raise UserError(
                    _(
                        "You are trying to delete a record that is still referenced by '%s' model!"
                        % m
                    )
                )
