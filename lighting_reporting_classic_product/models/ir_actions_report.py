...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):  # noqa: C901
        def close_streams(streams):
            for stream in streams:
                try:
                    stream.close()
                except Exception:
                    _logger.exception("Exception during stream closing")

        result = super(IrActionsReport, self)._render_qweb_pdf(
            report_ref, res_ids=res_ids, data=data
        )

        if self.report_name == "lighting_reporting_classic_product.report_classic":
