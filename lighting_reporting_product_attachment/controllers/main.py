...
class ProductDatasheetAttachmentController(ProductDatasheetController):
    def generate_lighting_report(self, company, template, product_id, lang_id=None):
        default_lang = "en_US"
        attachments_selected = product_id.attachment_ids.filtered(
            lambda x: x.use_as_product_datasheet
        )

        if not attachments_selected:
            return super().generate_lighting_report(
                company, template, product_id, lang_id=lang_id
            )

        attachments = attachments_selected.filtered(
            lambda x: (
                (not lang_id and (not x.lang_id or x.lang_id.code == default_lang))
                or (lang_id and (not x.lang_id or lang_id.code == x.lang_id.code))
            )
        )
        ...
        attachment = attachments.sorted(
            lambda x: (not x.lang_id and 1 or 0, x.sequence, x.id)
        )[0]
