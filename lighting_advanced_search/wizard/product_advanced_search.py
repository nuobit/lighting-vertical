...
class LightingProductAdvancedSearch(models.TransientModel):
    ...
    attachment_type_in_op = fields.Selection(
        string="Operator", selection=OP_SEL, default="and"
    )
    ...
    # application search
    application_in_ids = fields.Many2many(
        string="Applications",
    ...
    # finish search
    finish_in_ids = fields.Many2many(
        string="Finishes",
    ...
    # lampholder search
    lampholder_in_ids = fields.Many2many(
        string="Lampholders",
    ...
    # source type search
    source_type_in_ids = fields.Many2many(
        string="Source types",
    ...
    body_material_in_ids = fields.Many2many(
    ...
    voltage_in_ids = fields.Many2many(
        string="Voltages",
    ...
    # IP search
    sealing_in_ids = fields.Many2many(
        string="Sealing",
    ...
    color_temperature_in_ids = fields.Many2many(
    ...
    def advanced_search(self):
        ...
        # application
        domain += prepare_in_domain(
            ["application_ids"], self.application_in_ids, self.application_in_op
        )
        ...
        domain += prepare_in_domain(
            ["source_ids.lampholder_id", "source_ids.lampholder_technical_id"],
            self.lampholder_in_ids,
            self.lampholder_in_op,
        )

        # source type
        domain += prepare_in_domain(
            ["source_ids.line_ids.type_id"],
            self.source_type_in_ids,
            self.source_type_in_op,
        )

        # body material
        domain += prepare_in_domain(
            ["body_material_ids"], self.body_material_in_ids, self.body_material_in_op
        )

        # input/output voltage
        domain += prepare_in_domain(
            ["input_voltage_id", "output_voltage_id"],
            self.voltage_in_ids,
            self.voltage_in_op,
        )
        ...
        if self.wattage_from_in:
            if self.wattage_to_in:
                domain += [
                    ("source_ids.line_ids.wattage", ">=", self.wattage_from_in),
        ...
        if self.attachment_type_in_ids:
            for attachment_type_id in self.attachment_type_in_ids:
                domain_in.append(
                    ("attachment_ids.type_id.id", "=", attachment_type_id.id)
                )
