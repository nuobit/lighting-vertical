...
class LightingAttachmentPackage(models.Model):
    ...
    def generate_attach_zipfile(self):
        ...
        attachs = (
            ...
            .sorted(
                lambda x: (
                    x.product_id.family_ids.mapped("name")[:1],
        ...
        with tempfile.TemporaryFile() as tf:
            zf = zipfile.ZipFile(tf, mode="w", compression=zipfile.ZIP_DEFLATED)
            for a in attachs:
                zf.write(
                    self.env["ir.attachment"]._full_path(a.attachment_id.store_fname),
                    arcname=a.datas_fname,
                )
            zf.close()
            tf.seek(0)
            file_bin = tf.read()
    ...
    def generate_pdf_zipfile(self):
        ...
        products = (
            ...
            .search(domain)
            .sorted(lambda x: (x.family_ids.mapped("name")[:1], x.reference))
        )
        with tempfile.TemporaryFile() as tf:
            zf = zipfile.ZipFile(tf, mode="w", compression=zipfile.ZIP_DEFLATED)
            N = len(products)
            for i, p in enumerate(products):
                family_name = (
                    p.family_ids.mapped("name")
                    and p.family_ids.mapped("name")[0].upper()
                    or None
                )
                fname = "%s.pdf" % "_".join(
                ...
                _logger.info(
                    "Generating zip file '%s' with datasheet pdf's %i/%i"
